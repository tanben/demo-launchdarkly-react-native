...
function  initializeLDClient({mobileKey, userKey, ldClient, updateFlagHandler , flags={}}){
    const ldConfig = {
        mobileKey: mobileKey,
...
const AppLDProvider=  (props)=>{
    ...
    useEffect(() => {
        initializeLDClient({...props.config, ldClient, updateFlagHandler,flags})
        .catch(({message})=>{
            ldClient.flush();
            ldClient.close();
            // console.log(`${message}`)        
    ...
    return (
        <AppLDContext.Provider value={{...flags, likeButtonClickHandler, likeCount}}>
            {props.children}
